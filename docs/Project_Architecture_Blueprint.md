...
(1)                  (many-to-many)              (1)
                       ...
                       Account (archivedAt)    (pivot)
                       Device (archivedAt)──── DeviceCatalog*
                       UserSettings              │
                       Assessment ─── AssessmentAnswer
                       ChecklistItem
                       Incident ─── IncidentNote
                       Report
                       Booking (status) ─── BookingNote
                           │           └── BookingEvent
                           └── ServicePackage
...
*DeviceCatalog: DeviceCatalogDeviceType, DeviceCatalogBrand,
               DeviceCatalogModel, DeviceCatalogOsFamily, DeviceCatalogOsVersion
```
...
| Incident → IncidentNote | 1:many | IncidentNote.IncidentId |
| Family → Report | 1:many | Report.FamilyId |
...
| Booking → BookingNote | 1:many | BookingNote.BookingId |
| Booking → BookingEvent | 1:many | BookingEvent.BookingId |
...
| `BookingStatus` | `Draft`, `Submitted`, `Confirmed`, `InProgress`, `Completed`, `Cancelled` | string |
...
| `AddChecklistDueAtAndHelpUrl` | 2026-03-24 | `due_at` and `help_url` columns on checklist_items |
| `AddReports` | 2026-03-24 | Reports table |
| `AddUserSettings` | 2026-03-24 | UserSettings table (notification preferences per user) |
| `AddAccountArchivedAt` | 2026-03-25 | `archived_at` soft-delete on accounts |
| `AddDeviceArchivedAt` | 2026-03-25 | `archived_at` soft-delete on devices |
| `AddBookingStatus` | 2026-03-26 | `status` column on bookings (booking lifecycle state machine) |
| `AddUserStatusAndVerification` | 2026-03-27 | `status`, `email_verified_at` columns on users |
| `AddPasswordResetFields` | 2026-03-27 | `password_reset_token`, `password_reset_expires_at` on users |
| `AddFamilyNotes` | 2026-03-28 | `notes` column on families |
| `AddBookingAssignAndNotes` | 2026-03-28 | `assigned_to_id` + BookingNotes table |
| `AddIncidentNotes` | 2026-03-28 | IncidentNotes table |
| `AddServicePackageManagementFields` | 2026-03-28 | `is_active`, management fields on service_packages |
| `AddAdminNotes` | 2026-03-28 | AdminNotes table (general-purpose admin annotations) |
| `AddDeviceCatalogAndDeviceFKs` | 2026-04-01 | Device catalog tables (types, brands, models, OS families/versions); FK on devices |
| `RefactorBookingPaymentDomain` | 2026-04-03 | PaymentOrders table; refactor payment fields out of bookings |
| `AddPaymentOrderUrlFields` | 2026-04-03 | `payment_url`, `qr_code_url` on payment_orders |
...
| `IncidentStatusUpdated` | After `PATCH /api/incidents/{id}/status` |
| `BookingCreated` | After successful `POST /api/bookings` |
| `BookingSubmitted` | After `POST /api/bookings/{id}/submit` |
| `PaymentInitiated` | After `POST /api/bookings/{id}/payment/initiate` |
| `PaymentRetried` | After `POST /api/bookings/{id}/payment/retry` |
| `AdminCustomerNoteAdded` | `POST /api/admin/customers/{familyId}/notes` |
| `AdminBookingStatusChanged` | `PATCH /api/admin/bookings/{id}/status` |
| `AdminBookingPaymentStatusChanged` | `PATCH /api/admin/bookings/{id}/payment-status` |
| `AdminBookingAssigned` | `PATCH /api/admin/bookings/{id}/assign` |
| `AdminBookingNoteAdded` | `POST /api/admin/bookings/{id}/notes` |
| `AdminBookingReportAttached` | `PUT /api/admin/bookings/{id}/report` |
| `AdminIncidentStatusChanged` | `PATCH /api/admin/incidents/{id}/status` |
| `AdminIncidentNoteAdded` | `POST /api/admin/incidents/{id}/notes` |
| `AdminServicePackageCreated` | `POST /api/admin/service-packages` |
| `AdminServicePackageUpdated` | `PUT /api/admin/service-packages/{id}` |
| `AdminServicePackageStatusUpdated` | `PATCH /api/admin/service-packages/{id}/status` |
| `AdminUserStatusChanged` | `PATCH /api/admin/users/{id}/status` |
| `AdminUserRoleChanged` | `PATCH /api/admin/users/{id}/role` |
| `AdminPasswordResetTriggered` | `POST /api/admin/users/{id}/trigger-password-reset` |
| `AdminNoteCreated` | `POST /api/admin/notes` (AdminNotesPage) |
...
| `POST` | `/api/auth/logout` | None | — | Logout (clears session cookie) |
| `GET` | `/api/auth/me` | Required | — | Get current user (auto-refreshes claims) |
...
| `GET` | `/api/families/me` | Required | Get own family with members |
...
| `GET` | `/api/accounts/summary` | Required | Account summary counts |
| `GET` | `/api/accounts/{id}` | Required | Get account detail |
| `POST` | `/api/accounts` | Required | Create account |
| `PUT` | `/api/accounts/{id}` | Required | Update account |
| `DELETE` | `/api/accounts/{id}` | Required | Archive account (soft delete) |
...
| `GET` | `/api/devices/summary` | Required | Device summary counts |
| `GET` | `/api/devices/{id}` | Required | Get device detail |
| `POST` | `/api/devices` | Required | Register device |
| `PUT` | `/api/devices/{id}` | Required | Update device |
| `DELETE` | `/api/devices/{id}` | Required | Archive device (soft delete) |

#### Device Catalog (reference data)

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/device-catalog/device-types` | Required | All device type categories |
| `GET` | `/api/device-catalog/brands` | Required | Brands (optionally filtered by `deviceTypeCode`) |
| `GET` | `/api/device-catalog/models` | Required | Models (optionally filtered by `deviceTypeCode`, `brandCode`) |
| `GET` | `/api/device-catalog/os-families` | Required | OS families (optionally scoped by `modelCode`) |
| `GET` | `/api/device-catalog/os-versions` | Required | OS versions for a given `osFamilyCode` (required) |
| `GET` | `/api/device-catalog/form-options` | Required | All dropdown data in one call (reduces round-trips) |

#### Assessments

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/assessments/questions` | Required | Get full question bank |
| `POST` | `/api/assessments` | Required | Submit completed assessment |
| `GET` | `/api/assessments/latest` | Required | Latest assessment result |
| `GET` | `/api/assessments/history` | Required | Full assessment history |

#### Checklists

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/checklists` | Required | Get checklist items (filter by `severity`, `status`, `category`, `search`) |
| `GET` | `/api/checklists/summary` | Required | Checklist completion summary counts |
...
| Method | Path | Auth | Rate Limit | Description |
|--------|------|------|-----------|-------------|
| `GET` | `/api/incidents` | Required | — | List family incidents |
| `GET` | `/api/incidents/{id}` | Required | — | Get incident detail |
| `POST` | `/api/incidents` | Required | `mutations` | Report an incident |
| `PATCH` | `/api/incidents/{id}/status` | Required | `mutations` | Update incident status |

#### Reports

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/reports` | Required | List reports (filter by `reportType`, `search`, `fromDate`, `toDate`) |
| `GET` | `/api/reports/summary` | Required | Report counts by type |
| `GET` | `/api/reports/{id}` | Required | Get report detail |
| `GET` | `/api/reports/{id}/download` | Required | Get download URL + metadata |

#### Bookings & Service Packages

| Method | Path | Auth | Rate Limit | Description |
|--------|------|------|-----------|-------------|
| `GET` | `/api/service-packages` | Required | — | List available service packages |
| `POST` | `/api/bookings` | Required | `mutations` | Create a Draft booking |
| `POST` | `/api/bookings/{id}/submit` | Required | `mutations` | Submit booking (Draft → Submitted/Confirmed) |
| `GET` | `/api/bookings/my` | Required | — | List own bookings |
| `GET` | `/api/bookings/summary` | Required | — | Booking summary counts |
| `GET` | `/api/bookings/{id}` | Required | — | Get booking detail |
| `GET` | `/api/bookings/{id}/events` | Required | — | Booking event timeline |

#### Payments

| Method | Path | Auth | Rate Limit | Description |
|--------|------|------|-----------|-------------|
| `POST` | `/api/bookings/{bookingId}/payment/initiate` | Required | `mutations` | Initiate payment (returns checkout URL / QR) |
| `POST` | `/api/bookings/{bookingId}/payment/retry` | Required | `mutations` | Retry a failed payment |
| `GET` | `/api/bookings/{bookingId}/payments` | Required | — | List payment orders for a booking |
| `POST` | `/api/webhooks/payment/{provider}` | None | — | Gateway webhook receiver (HMAC-verified) |

#### Settings

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/api/settings` | Required | Get profile + notification settings |
| `PUT` | `/api/settings/profile` | Required | Update display name and phone |
| `PUT` | `/api/settings/notifications` | Required | Update notification preferences |
| `POST` | `/api/settings/change-password` | Required | Change password |
...
| `GET` | `/api/admin/customers` | Admin | All customer families |
| `GET` | `/api/admin/customers/{familyId}` | Admin | Customer family detail |
| `POST` | `/api/admin/customers/{familyId}/notes` | Admin | Add note to customer |
...
| `PATCH` | `/api/admin/bookings/{id}/payment-status` | Admin | Update booking payment status |
| `PATCH` | `/api/admin/bookings/{id}/assign` | Admin | Assign booking to staff |
| `POST` | `/api/admin/bookings/{id}/notes` | Admin | Add note to booking |
| `PUT` | `/api/admin/bookings/{id}/report` | Admin | Attach report to booking |
| `GET` | `/api/admin/incidents` | Admin | All incidents |
| `GET` | `/api/admin/incidents/{id}` | Admin | Incident detail |
| `PATCH` | `/api/admin/incidents/{id}/status` | Admin | Update incident status |
| `POST` | `/api/admin/incidents/{id}/notes` | Admin | Add note to incident |
| `GET` | `/api/admin/audit-logs` | Admin | Audit log entries |
| `GET` | `/api/admin/reports` | Admin | All family reports |
| `GET` | `/api/admin/reports/{id}` | Admin | Report detail |
| `GET` | `/api/admin/reports/{id}/download` | Admin | Report download URL |
| `GET` | `/api/admin/service-packages` | Admin | List service packages |
| `POST` | `/api/admin/service-packages` | Admin | Create service package |
| `PUT` | `/api/admin/service-packages/{id}` | Admin | Update service package |
| `PATCH` | `/api/admin/service-packages/{id}/status` | Admin | Activate/deactivate package |
| `GET` | `/api/admin/users` | Admin | All user accounts |
| `GET` | `/api/admin/users/{id}` | Admin | User detail |
| `PATCH` | `/api/admin/users/{id}/status` | Admin | Suspend/reactivate user |
| `PATCH` | `/api/admin/users/{id}/role` | Admin | Change user role |
| `POST` | `/api/admin/users/{id}/trigger-password-reset` | Admin | Force password reset |
| `GET` | `/api/admin/activity` | Admin | System activity feed |
...
| Payment gateways | Custom (`IPaymentGateway`) | payOS, MoMo, ZaloPay, Mock |
| Webhook security | HMAC-SHA256 per provider | Signature verification for payment events |
