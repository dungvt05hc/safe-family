...
import { useAdminCustomers } from '../customers/adminCustomers.hooks'
import type { AdminCustomerFilters } from '../customers/adminCustomers.types'
import { AdminCustomerFiltersBar } from '../customers/AdminCustomerFilters'
import { AdminCustomersTable } from '../customers/AdminCustomersTable'
import { AdminSpinner, AdminError, AdminEmpty } from '../components/AdminStateViews'

const DEFAULT_FILTERS: AdminCustomerFilters = {
  search: '',
  riskLevel: '',
  planType: '',
  page: 1,
...
export function AdminCustomersPage() {
  const [filters, setFilters] = useState<AdminCustomerFilters>(DEFAULT_FILTERS)
  const { data, isLoading, isFetching, isError, refetch } = useAdminCustomers(filters)

  function handleFiltersChange(next: Partial<AdminCustomerFilters>) {
    setFilters((prev) => ({ ...prev, ...next }))
  }

  const totalPages = data ? Math.max(1, Math.ceil(data.total / filters.pageSize)) : 1
  const hasActiveFilters = filters.search !== '' || filters.riskLevel !== '' || filters.planType !== ''
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <AdminCustomerFiltersBar filters={filters} onChange={handleFiltersChange} />
        {hasActiveFilters && (
          <button
            type="button"
      ...
      {/* Pagination — always show count; only show buttons when there are multiple pages */}
      {data && (
