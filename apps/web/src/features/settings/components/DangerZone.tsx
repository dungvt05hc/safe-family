  ...
  if (done) {
    return (
      <Alert variant="success">
        {t('danger.deleteConfirm.success')}
      </Alert>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
      ...
      {deleteMutation.isError && (
        <Alert variant="error">{t('danger.deleteConfirm.error')}</Alert>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {t('danger.deleteConfirm.deleteButton')}
        </Button>
        <Button type="button" variant="ghost" size="sm" onClick={onCancel}>
          {t('danger.deleteConfirm.cancel')}
        </Button>
      </div>
    </form>
  )
  ...
  return (
    <div className="rounded-2xl border-2 border-red-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b border-red-100 bg-red-50 px-5 py-4">
        ...
        <h2 className="text-sm font-semibold text-red-700">{t('danger.zoneTitle')}</h2>
      </div>
      ...
      <div className="px-5 py-5 space-y-4">
        {!confirming ? (
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-medium text-gray-800">{t('danger.deleteAccount.title')}</p>
